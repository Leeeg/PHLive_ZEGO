...
public class NotificationUtil {
     ...
     * @param context 上下文
     */
    public static void showNotification(String tick, String title, String content, ChannelData channelData, Context context) {
        Intent intent = new Intent(context, WelcomeActivity.class);
        intent.putExtra("position", 0);
        intent.putExtra("isNotification", true);
        intent.putExtra("cId", channelData.getCId());
        intent.putExtra("HttpPullUrl", channelData.getHttpPullUrl());
        intent.putExtra("CoverUrl", channelData.getCoverUrl());
        intent.putExtra("RoomId", channelData.getYunXinRId());
...
//                .setTicker("有新资讯")
//                .setPriority(MyNotification.PRIORITY_HIGH)// 设置该通知优先级
//                .setOngoing(false)//不是正在进行的   true为正在进行  效果和.flag一样
//                .setSmallIcon(R.mipmap.icon);
//        MyNotification notify = mBuilder.build();
//        NotificationManager notificationManager = (NotificationManager) context.getSystemService(context.NOTIFICATION_SERVICE);
