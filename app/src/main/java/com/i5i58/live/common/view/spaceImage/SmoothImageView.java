 ...
 * 仅限于用�?:从一个ScaleType==CENTER_CROP的ImageView，切换到另一个ScaleType=
 * FIT_CENTER的ImageView，或者反�? (当然，得使用同样的图片最�?)
 * 
 * @author Dean Tao
 * 
 */
...
//		setBackgroundColor(mBgColor);
	...
	public void setOriginalInfo(int width, int height, int locationX, int locationY) {
		...
		// 因为是屏幕坐标，�?以要转换为该视图内的坐标，因为我�?用的该视图是MATCH_PARENT，所以不用定位该视图的位�?,如果不是的话，还�?要定位视图的位置，然后计算mOriginalLocationX和mOriginalLocationY
	...
	/**
	 * 用于�?始进入的方法�? 调用此方前，�?已经调用过setOriginalInfo
	 */
	...
	/**
	 * 用于�?始�??出的方法�? 调用此方前，�?已经调用过setOriginalInfo
	 */
	...
	private class Transfrom {
		float startScale;// 图片�?始的缩放�?
		float endScale;// 图片结束的缩放�??
		float scale;// 属�?�ValueAnimator计算出来的�??
		LocationSizeF startRect;// �?始的区域
		LocationSizeF endRect;// 结束的区�?
		LocationSizeF rect;// 属�?�ValueAnimator计算出来的�??
	...
	private void initTransform() {
		...
		/** 下面为缩放的计算 */
		/* 计算初始的缩放�?�，初始值因为是CENTR_CROP效果，所以要保证图片的宽和高至少1个能匹配原始的宽和高，另1个大�? */
		...
		/* 计算结束时�?�的缩放值，结束值因为要达到FIT_CENTER效果，所以要保证图片的宽和高至少1个能匹配原始的宽和高，另1个小�? */
		...
		/**
		 * 下面计算Canvas Clip的范围，也就是图片的显示的范围，因为图片是慢慢变大，并且是等比例的，�?以这个效果还�?要裁减图片显示的区域
		 * ，�?�显示区域的变化范围是在原始CENTER_CROP效果的范围区�?
		 * ，到�?终的FIT_CENTER的范围之间的，区域我用LocationSizeF更好计算
		 * ，他就包括左上顶点坐标，和宽高，�?后转为Canvas裁减的Rect.
		 */
		/* �?始区�? */
		...
		float bitmapEndWidth = mBitmap.getWidth() * mTransfrom.endScale;// 图片�?终的宽度
		float bitmapEndHeight = mBitmap.getHeight() * mTransfrom.endScale;// 图片�?终的宽度
	...
	/* 下面实现了CENTER_CROP的功�? 的Matrix，在优化的过程中，已经不用了 */
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mState == STATE_TRANSFORM_IN || mState == STATE_TRANSFORM_OUT) {
			...
			//当Transform In变化完成后，把背景改为黑色，使得Activity不�?�明
			mPaint.setAlpha(255);
			canvas.drawPaint(mPaint);
			super.onDraw(canvas);
		}
	...
	private void startTransform(final int state) {
		...
		valueAnimator.addListener(new ValueAnimator.AnimatorListener() {
			...
			public void onAnimationEnd(Animator animation) {
				 ...
				 * �? 而应该是�?后变化的位置，因为当out的时候结束时，不回复视图是Normal，要不然会有�?个突然闪动回去的bug
				 */
				// TODO 这个可以根据实际�?求来修改
				if (state == STATE_TRANSFORM_IN) {
					mState = STATE_NORMAL;
