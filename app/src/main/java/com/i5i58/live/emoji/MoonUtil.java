...
public class MoonUtil {
	...
	public static void identifyFaceExpressionA(Context context, View textView, MyRichText value,  int align) {
		identifyFaceExpressionA(context, textView, value, align, DEF_SCALE);
	}
	...
	public static void identifyFaceExpressionA(Context context, View textView, MyRichText value, int align, float scale) {
		SpannableString mSpannableString = replaceEmoticonsA(context, value, scale, align);
		for (int i = 0; i < value.getSpans().size(); i++) {
			mSpannableString.setSpan(value.getSpans().get(i).getSpan(), value.getSpans().get(i).getStart(),
					value.getSpans().get(i).getEnd(), value.getSpans().get(i).getFlag());
		}
		viewSetText(textView, mSpannableString);
	}
	 ...
	 */
	public static void identifyFaceExpressionAndTags(Context context, View textView, String value, int align,
			float scale) {
	...
	private static SpannableString replaceEmoticonsA(Context context, MyRichText value, float scale, int align) {
		if (TextUtils.isEmpty(value.getText())) {
			value.setText("") ;
		}
		
		SpannableString mSpannableString = new SpannableString(value.getText());
		Matcher matcher = EmojiManager.getPattern().matcher(value.getText());
		while (matcher.find()) {
			int start = matcher.start();
			int end = matcher.end();
			String emot = value.getText().substring(start, end);
			Drawable d = getEmotDrawable(context, emot, scale);
			if (d != null) {
				ImageSpan span = new ImageSpan(d, align);
				mSpannableString.setSpan(span, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
