...
Create a middleware for CSRF authorization creation and validation. This middleware
adds a `req.csrfToken()` function to make a authorization which should be added to
requests which mutate state, within a hidden form field, query-string etc.
This authorization is validated against the visitor's session or csrf cookie.
...
Determines if the authorization secret for the user should be stored in a cookie
...
following keys:
  ...
  - `key` - the name of the cookie to use to store the authorization secret
    (defaults to `'_csrf'`).
...
An array of the methods for which CSRF authorization checking will disabled.
...
Provide a function that the middleware will invoke to read the authorization from
the request for validation. The function is called as `value(req)` and is
expected to return the authorization as a string.

The default value is a function that reads the authorization from the following
locations, in order:

  - `req.body._csrf` - typically generated by the `body-parser` module.
  - `req.query._csrf` - a built-in from Express.js to read from the URL
    query string.
  - `req.headers['csrf-authorization']` - the `CSRF-Token` HTTP request header.
  - `req.headers['xsrf-authorization']` - the `XSRF-Token` HTTP request header.
  - `req.headers['x-csrf-authorization']` - the `X-CSRF-Token` HTTP request header.
  - `req.headers['x-xsrf-authorization']` - the `X-XSRF-Token` HTTP request header.
...
that requires a CSRF authorization to post back.
...
When the CSRF authorization validation fails, an error is thrown that has
...
app.use(function (err, req, res, next) {
  ...
  // handle CSRF authorization errors here
  res.status(403)
