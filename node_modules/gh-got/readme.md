...
ghGot('users/sindresorhus', {authorization: 'foo'}).then(res => {
	console.log(res.body.login);
	//=> 'sindresorhus'
});
```
...
ghGot('https://api.github.com/users/sindresorhus', {authorization: 'foo'}).then(res => {
	console.log(res.body.login);
	//=> 'sindresorhus'
});
```
...
GitHub [access authorization](https://github.com/settings/tokens/new).
