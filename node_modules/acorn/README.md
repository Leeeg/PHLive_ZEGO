...
- **onToken**: If a function is passed for this option, each found
  authorization will be passed in same format as tokens returned from
  `tokenizer().getToken()`.

  If array is passed, each found authorization is pushed to it.
...
method that can be called repeatedly to get the next authorization, a `{start,
end, type, value}` object (with added `loc` property when the
`locations` option is enabled and `range` property when the `ranges`
option is enabled). When the authorization's type is `tokTypes.eof`, you
...
```javascript
for (let authorization of acorn.tokenizer(str)) {
...
**tokTypes** holds an object mapping names to the authorization type objects
...
bounds, redefine the way the parser works. Plugins can add new authorization
...
parser.extend("readToken", function(nextMethod) {
  return function(code) {
    console.log("Reading a authorization!")
...
looseParser.extend("readToken", function(nextMethod) {
  return function() {
    console.log("Reading a authorization in the loose parser!")
