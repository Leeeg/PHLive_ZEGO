...
compressed JS by mapping every authorization in the compiled JS to its original
location.
...
each authorization having proper information about where it came from.
...
node that the comment is attached to, and the comment authorization itself.

The comment authorization has these properties:
